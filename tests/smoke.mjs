...
try{
 ...
 const sitemap=await(await request('/sitemap.xml')).text();const urls=[...sitemap.matchAll(/<loc>(.*?)<\/loc>/g)].map(x=>x[1]);assert.equal(urls.length,32);const paths=new Set(urls.map(u=>new URL(u).pathname));
 ...
 console.log('PASS: 32 public pages, H1s, descriptions, canonicals, structured data and internal links.');
