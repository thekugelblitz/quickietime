...
const plan=z.object({id:z.string().uuid().optional(),name:z.string().min(2).max(80),description:z.string().max(400),credits:z.number().int().min(1).max(1000000),amount:z.number().int().min(100).max(100000000),currency:z.enum(['INR','USD']),active:z.boolean()});
...
export async function POST(r:Request,{params}:{params:Promise<{action:string}>}){
  ...
  try{
    ...
    if(action==='plans'){const v=plan.parse(body),id=v.id||crypto.randomUUID();d.prepare('INSERT INTO plans VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,description=excluded.description,credits=excluded.credits,amount=excluded.amount,currency=excluded.currency,active=excluded.active').run(id,v.name,v.description,v.credits,v.amount,v.currency,Number(v.active),new Date().toISOString());audit(a.id,'plan_saved',id);return reply({ok:true,id})}
